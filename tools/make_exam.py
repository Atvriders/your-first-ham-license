"""Practice-exam generator for Your First Ham License.

Draws a valid Technician class practice exam from the structured question
pool (``canon/pool-technician.json``): exactly one question per NCVEC group
...
answer, figure}}`` — ids like ``T1A01`` … ``T0F##``.
"""
...
def render_key(questions: list[Question]) -> str:
    ...
    for sub in sorted(tally, key=lambda s: (10 if s == "T0" else int(s[1:]) if s[1:].isdigit() else 99, s)):
...
def main(argv=None) -> int:
    parser = argparse.ArgumentParser(
        description="Draw a Technician class practice exam from the question pool.")
