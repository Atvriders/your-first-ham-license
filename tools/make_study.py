"""Interactive study pages for Your First Ham License.

Generates two self-contained, CSP-safe HTML pages from the canonical pool
data — everything (pool JSON, redrawn figure SVGs, CSS, JS) inlined, no
external references:

  practice.html    -- draws a valid Technician exam: exactly one question
  ...
  flashcards.html  -- all 409 questions as flippable cards: front = question
                      ...
                      mark persisted in localStorage ("yfhl-study").
...
``canon/pool-technician.txt``, and ``chapter`` follows the subelement ->
chapter map of ``accuracy-canon.md`` §5 (T8 splits: T8A -> ch02, T8B–T8D ->
ch07). The 12 figure questions get their redrawn pool figure SVG
(``figures/ch01-pool-fig-t{1,2,3}.svg``) embedded inline.

Usage:
  python3 tools/make_study.py [--out build/]
      [--pool canon/pool-technician.json] [--appendix appendices/pool.md]
      [--pool-txt canon/pool-technician.txt] [--figures-dir figures]
"""
...
DEFAULT_POOL_PATH = "canon/pool-technician.json"
DEFAULT_APPENDIX_PATH = "appendices/pool.md"
DEFAULT_POOL_TXT_PATH = "canon/pool-technician.txt"
DEFAULT_FIGURES_DIR = "figures"

POOL_QUESTION_COUNT = 409
...
# Redrawn pool figures (figures/figures.json: kind "original", redrawn from
# the NCVEC pool figures).
FIGURE_FILES = {
    "T-1": "ch01-pool-fig-t1.svg",
    "T-2": "ch01-pool-fig-t2.svg",
    "T-3": "ch01-pool-fig-t3.svg",
}

# Subelement -> teaching chapter (accuracy-canon.md §5). T8 is split:
# ch02 owns T8A only; ch07 owns T8B, T8C, T8D.
_SUBELEMENT_CHAPTER = {
    "T1": 8, "T2": 6, "T3": 3, "T4": 5, "T5": 1,
    "T6": 1, "T7": 5, "T9": 4, "T0": 9,
}

_GROUP_HEADING_RE = re.compile(r"^(T\d[A-F]) (\S.*\S)\s*$")
_SUBELEMENT_RE = re.compile(r"^SUBELEMENT (T\d) - (.+?) \[\d+ Exam Questions? - \d+ Groups?\]\s*$")
_ENTRY_ID_RE = re.compile(r"^> \*\*(T\d[A-F]\d\d)\*\*")
_ANSWER_RE = re.compile(r"^> \*\*Answer: ([A-D])\*\* — (.*\S)\s*$")


def parse_group_headings(pool_txt: str) -> dict:
    """Published group headings from the canonical pool text: ``T1A`` ->
    "Purpose and permissible use ...". Question ID lines (``T1A01 (C)``)
    never match: a digit, not a space, follows the group letter there."""
    headings = {}
...
def parse_subelement_titles(pool_txt: str) -> dict:
    """Published subelement titles: ``T1`` -> ``COMMISSION’S RULES`` (case as
    published in the pool document)."""
    titles = {}
    for line in pool_txt.splitlines():
...
def load_figures(figures_dir) -> dict:
    """The three redrawn pool figures, inlined: ``T-1`` -> SVG markup."""
    figures = {}
...
def subelement_summaries(records: list, titles: dict) -> list:
    """Per-subelement counts + published titles, ordered T1–T9 then T0."""
    counts: dict[str, int] = {}
    for rec in records:
        counts[rec["subelement"]] = counts.get(rec["subelement"], 0) + 1
    order = sorted(counts, key=lambda s: 10 if s == "T0" else int(s[1:]))
...
_SERIES_BAR = """<nav class="series-bar" aria-label="Books in this series">
  <a class="current" href="/tech/" aria-current="page">Technician</a>
...
<p class="rule">The real Technician exam draws <b>35 questions</b> from this pool — one
