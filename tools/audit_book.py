"""Verification harness ("test suite") for the finished Your First Ham License book.

Cross-checks the book against the accuracy canon, the 2026–2030 Technician
...
depend on missing inputs are skipped, not failed):
    ...
    canon/pool-technician.json -- structured pool: id -> {group, subelement,
...
POOL_JSON_PATH = "canon/pool-technician.json"
...
_POOL_ID_RE = re.compile(r"^T(\d)([A-F])(\d\d)$")
...
def check_format_laws(stem: str, text: str) -> list[str]:
    ...
    the spec §5 skeleton:
      ...
      - teaching chapters ch01–ch09 carry ≥1 ``> **Worked example:**``
        blockquote and a ``### Exam Focus`` section; ch00/ch10 must NOT have
...
def extract_pool_quotes(text: str) -> list[dict]:
    ...
    Each block starts with a line ``> **T#X##** <question>`` and continues
...
def pool_sort_key(qid: str) -> tuple:
    """Canonical pool order: subelements T1–T9 then T0, group A–F, number."""
...
def main() -> int:
    ...
    print("=== Your First Ham License: book audit ===\n")
